...
test("an implicit function called with inconsistent arity is YS0014", () => {
  ...
  assert.ok(
    result.diagnostics.some((d) => d.code === "YS0014"),
    `expected YS0014, got ${codesOf(result)}`,
  );
  checkAgainstDefinitions(result);
});
...
test("a variable typed elsewhere resolves for line use", () => {
  const declared = compileSource(
    "title: Start\n---\n<<declare $myVar = 1>>\nHere: {$myVar}\n===\n",
  );
  assertClean(declared);
  const setFirst = compileSource(
    "title: Start\n---\n<<set $x = 1>>\nHere: {$x}\n===\n",
  );
