...
`,
  ...
  const texts = Object.values(result.stringTable!)
    .map((e) => e.text)
    .sort();
...
`,
  ...
  assert.equal(
    ...
    false,
    placeholderEqual.diagnostics
      .filter((d) => d.code === "YS0044")
      .map((d) => d.message)
      .join("; "),
  );
...
`,
  ...
  assert.ok(
    differing.diagnostics.some((d) => d.code === "YS0044"),
    "differing text still reports YS0044",
  );
});
...
function germanProvider() {
  ...
  provider.extendTranslation(
    "de",
    Object.fromEntries(translated.map((e) => [e.id, e.text])),
  );
...
test("a provider's base-language table (placeholder text) composes the same values", () => {
  ...
  const baseTable = Object.fromEntries(
    Object.entries(result.stringTable!).map(([id, info]) => [
      id,
      info.text ?? "",
    ]),
  );
