...
const lineOf = (
  event: DialogueEvent | undefined,
): Extract<DialogueEvent, { type: "line" }> | undefined =>
...
`;
  ...
  strictEqual(
    first[0].type,
    "nodeStart",
    "Expected the node-start event first",
  );
  ...
  strictEqual(
    third[third.length - 1].type,
    "dialogueComplete",
    "Expected dialogue completion",
  );
});
...
===
  ...
  strictEqual(
    markup.text.slice(bold.position, bold.position + bold.length),
    "bold",
  );
  ...
  strictEqual(
    markup.text.slice(wave.position, wave.position + wave.length),
    "custom",
  );
