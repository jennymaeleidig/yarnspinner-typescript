...
test("plain set: to and = spellings, spaced", () => {
  assert.deepEqual(parseStateStatement("set $gold to 5"), {
    kind: "set", name: "gold", expression: "5", assignment: "to",
  });
  assert.deepEqual(parseStateStatement("set $gold = 5"), {
    kind: "set", name: "gold", expression: "5", assignment: "=",
  });
  assert.deepEqual(parseStateStatement("set  $gold   to   $gold + 1"), {
    kind: "set", name: "gold", expression: "$gold + 1", assignment: "to",
...
test("compound set: every operator, spaced and attached", () => {
  for (const op of ["+=", "-=", "*=", "/=", "%="] as const) {
    ...
    assert.deepEqual(spaced, {
      kind: "set", name: "n", expression: "1", compoundOp: op,
    }, `spaced ${op}`);
...
test("declare: with and without the as-TYPE postfix", () => {
  assert.deepEqual(parseStateStatement("declare $x = 0 as number"), {
    kind: "declare", name: "x", expression: "0", assignment: "=", declaredType: "number",
  });
  assert.deepEqual(parseStateStatement("declare $x = 0"), {
    kind: "declare", name: "x", expression: "0", assignment: "=",
  });
  assert.deepEqual(parseStateStatement("declare $x=$y"), {
    kind: "declare", name: "x", expression: "$y", assignment: "=",
  }, "the declare `=` may be attached");
  assert.deepEqual(parseStateStatement('declare $s = "hello" as string'), {
    kind: "declare", name: "s", expression: '"hello"', assignment: "=", declaredType: "string",
  });
});
...
test("identifiers follow the upstream rule (the old \\w+ looseness is gone)", () => {
  assert.equal(parseStateStatement("set $1abc to 1"), null,
...
test("non-statements parse to null", () => {
  ...
  assert.equal(parseStateStatement("set $x"), null, "operator/expression missing");
  assert.equal(parseStateStatement("set x to 1"), null, "the variable reference carries $");
  assert.equal(parseStateStatement("set $x ="), null, "an operator with no expression");
