...
function zlibLineId(fileName: string, nodeName: string, count: number): string {
  const checksum = zlibCrc32(Buffer.from(`${fileName}${nodeName}${count}`, "utf8"));
...
`,
  ...
  assert.equal(sourceEntry.shadowLineID, null, "source lines do not have a shadow line ID");
  assert.deepEqual(sourceEntry.metadata, ["line:source", "apple"]);

  const shadowEntry = Object.entries(table).find(([id]) => id !== "line:source")![1];
  assert.ok(Object.keys(table).find((id) => id.startsWith("line:sh_")), "implicit shadow IDs carry the sh_ prefix");
  assert.equal(shadowEntry.text, null, "shadow lines do not contain any source text");
  assert.equal(shadowEntry.shadowLineID, "line:source", "the #shadow: tag resolves with the line: prefix");
  assert.equal(shadowEntry.isImplicitTag, true);
  assert.deepEqual(shadowEntry.metadata, ["shadow:source", "banana"], "shadow lines have their own metadata");
});
...
`,
  ...
  assert.equal(shadows.length, 1, "upstream UnknownLineIDForShadowLine fires in every mode");
  assert.match(shadows[0].message, /line:missing/);
  assert.equal(shadows[0].file, "input");
  const entry = Object.values(result.stringTable!).find((e) => e.shadowLineID === "line:missing");
  assert.ok(entry);
  assert.equal(entry!.text, "Line content", "an unresolvable shadow keeps its text (upstream continues past YS0042)");
});
...
`,
  ...
  const shadowEntry = Object.values(result.stringTable!).find((e) => e.shadowLineID === "line:abc123");
  assert.ok(shadowEntry);
  assert.equal(shadowEntry!.text, null, "upstream strips the text even when validation failed");
});
...
`,
  ...
  assert.equal(shadows.length, 1, "upstream ShadowLinesMustHaveSameTextAsSource");
  const shadowEntry = Object.values(result.stringTable!).find((e) => e.shadowLineID === "line:source");
  assert.ok(shadowEntry);
...
`,
  ...
  assert.equal(mixed.length, 2, "upstream reports the pair: the shadow tag then the line tag");
  assert.ok(Object.values(result.stringTable!).every((e) => e.text !== "both tags"), "the line registers no entry");
  assert.equal(result.containsImplicitStringTags, true, "the ordinary line still registered");
});
...
`,
  ...
  assert.deepEqual(result.stringTable!["line:choose"].metadata, ["line:choose", "lastline"]);
});
...
`,
  ...
  assert.deepEqual(result.stringTable!["line:choose"].metadata, ["line:choose"]);
});
...
`,
  ...
  assert.deepEqual(result.stringTable!["line:in_if"].metadata, ["line:in_if", "lastline"]);
  assert.deepEqual(result.stringTable!["line:in_once"].metadata, ["line:in_once"]);
});
...
`;
  ...
  const shadowID = Object.keys(result.stringTable!).find((id) => id.startsWith("line:sh_"))!;
  assert.ok(shadowID);
  const node = result.program!.nodes["Start"] as { instructions: Array<{ op: string; text?: string; tags?: string[] }> };
  const shadowIns = node.instructions.find((ins) => ins.op === "runLine" && ins.text === "This is a line." && ins.tags?.includes(shadowID));
  assert.ok(shadowIns, "the shadow line's runLine carries the sh_ ID the table registered");
...
`,
  ...
  assert.equal(result.containsImplicitStringTags, false, "the only implicit ID belongs to a shadow line");
});
