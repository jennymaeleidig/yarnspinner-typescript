...
import { applyBinaryOp, applyUnaryOp, deepEqualsOperands, stringifyOperand, toNumberOperand } from "../runtime/operands.js";
...
test("add: string concat with upstream rendering when either side is a string", () => {
  ...
  assert.equal(applyBinaryOp("add", true, "!"), "True!", "booleans render as C# ToString");
  assert.equal(applyBinaryOp("add", null, "x"), "x");
  assert.equal(applyBinaryOp("add", 1, 2), 3, "both numeric → numeric addition");
...
test("subtract/multiply/divide/modulo: numeric via toNumberOperand", () => {
  ...
  assert.equal(applyBinaryOp("subtract", null, 1), -1, "null/empty coerce to 0");
  assert.throws(() => applyBinaryOp("subtract", "abc", 1), /Cannot convert/, "a non-numeric string is an error, not NaN");
});

test("equalTo/notEqualTo: deep equality with implicit defaults for unset", () => {
  assert.equal(applyBinaryOp("equalTo", 1, 1), true);
  assert.equal(applyBinaryOp("equalTo", 1, "1"), false, "different types are not equal");
  assert.equal(applyBinaryOp("equalTo", undefined, false), true, "unset bool variable carries its default");
  ...
  assert.equal(applyBinaryOp("equalTo", { a: 1 }, { a: 1 }), true, "deep equality");
});
...
test("relationals: numeric via plain Number(), NaN comparisons yield false", () => {
  ...
  assert.equal(applyBinaryOp("lessThan", "abc", 1), false, "NaN < x is false, never a throw");
});

test("and/or/xor: boolean semantics over the values as given", () => {
  assert.equal(applyBinaryOp("and", true, true), true);
  assert.equal(applyBinaryOp("and", true, 0), false, "non-bool operands coerce");
  ...
  assert.equal(applyBinaryOp("xor", "a", "b"), false, "two truthy values xor to false");
});
