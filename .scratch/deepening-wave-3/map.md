...
- **Fog (doc-updater pass, 2026-09-04, independent of this wave)**: docs/compatibility.md's ternary Known-issues entry claims a ternary in `<<set>>` "parses without a diagnostic but silently evaluates to nothing" — verified false at both e2140dd and HEAD (the repro yields YS0006 Unclosed command at compile, a diagnostic). The claim predates wave 3 and isn't caused by it; needs its own correction or ticket.
