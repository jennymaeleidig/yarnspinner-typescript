...
- **Two-axis review fix pass (2026-09-04, whole-wave diff `e2140dd...HEAD`)**: Standards axis 6 met / 1 partial / 0 missing + 2 smells; Spec axis 6 met / 0 missing / 0 unrecorded creep / 1 partial. Fixes landed: (1) §7 — every `deepening-wave-3 ticket NN` citation in `src/` rewritten to living artifacts (docs/compatibility.md's fallback-execution divergence for ticket 01's sites, CONTEXT.md "stopping point" for ticket 04's) or dropped where the inline explanation carries the why; pre-existing wave-2 citations left as-is (out of this wave's diff, same convention problem — candidate for a future hygiene pass). (2) Duplicated Code smell — the `e instanceof Error ? e.message : String(e)` shape at ten catch sites extracted as `describeError` in `src/describeError.ts` (internal, not package surface). (3) Speculative Generality smell — `emptyCompileResult`'s unused `diagnostics` default removed (both call sites pass it). (4) Spec-axis partial — `runUntilCompleteEvents`' "empty-batch belt" comment was wrong (an empty batch can never exit `pullUntilStopped`'s loop, so empty `pulled` is exactly the at-rest guard firing); the docstring's phantom third terminal bullet deleted, the inline comment now states the real invariant. Suite 631 working-tree (630 pass, 1 mirrored skip — the concurrent agent's five index.test.ts tests still ride; committed-tree delta noted above), lint clean, ts-check clean.
