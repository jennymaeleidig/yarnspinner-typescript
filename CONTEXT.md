...
- **YarnProject**: an upstream-format `.yarnproject` (v4 schema, legacy v2 accepted; the dead dev v3 rejected) describing source files, localisation, and compiler options. Loaded via `loadProject`/`listSources` behind an injected file-access seam (Node provider under the `./node` subpath); loader diagnostics use this project's local `YP`-code range — upstream has no registry for project files.
