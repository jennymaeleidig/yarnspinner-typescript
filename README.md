...
* `useDialogue(program: Program, options?: UseDialogueOptions)` — React hook over `Dialogue`
  ...
  * Options: `startAt`, `functions`, `variables`, `onDialogueComplete` (fired once on dialogue completion, with the story variables; deprecated alias: `onStoryEnd`), `variableStorage` (the persistence seam — inject a pre-populated storage to restore state; a new storage identity rebuilds the dialogue), `textProvider` (line-ID → text for the current language; a new provider identity rebuilds the dialogue — switch languages via `dialogue.setLanguage` on the hook result, no rebuild), `lineHints` (opt-in `LineHintsEvent`; the hook consumes hints silently, so observe them via the provider's `acceptLineHints` or the `dialogue` escape hatch — flipping the flag rebuilds), `logError`/`logDebug` (runtime diagnostics; construction-time, changing them is ignored; defaults `console.error`/silent)
  * Deprecated aliases: `advance` (same function as `continue`)
* `<DialogueView program={...} startNode={...} scenes={...} onDialogueComplete={...} />` — Ready-to-use dialogue component; disables unavailable option buttons and fires `onDialogueComplete` once on dialogue completion (deprecated prop aliases: `onStoryEnd`, `autoAdvanceAfterTyping`/`autoAdvanceDelay`/`pauseBeforeAdvance` → `autoContinueAfterTyping`/`autoContinueDelay`/`pauseBeforeContinue`); also takes `variableStorage`, `textProvider`, `lineHints`, `logError`, and `logDebug` with the same rebuild-on-change behaviour as the hook
